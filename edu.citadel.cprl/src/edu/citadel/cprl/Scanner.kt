 ...
 */
class Scanner(
    private val source: Source, k: Int, private val errorHandler: ErrorHandler,
) {
    ...
    init {
        for (symbol in Symbol.entries) {
            if (symbol.isReservedWord()) reservedWords.add(symbol)
        }
        reservedWords.sort()
    ...
    fun advanceTo(symbol: Symbol) {
        while (this.symbol != symbol && this.symbol != Symbol.EOF) advance()
    }
    ...
    fun advanceTo(symbols: Set<Symbol>) {
        while (!symbols.contains(symbol) && symbol != Symbol.EOF) advance()
    }
    ...
    private fun searchForSymbol(searchString: String, symbols: List<Symbol>, low: Int, high: Int): Symbol? {
        if (low >= high) return null

        val mid = (low + high) / 2
        val midSymbol = symbols[mid]
        val label = midSymbol.label

        return when {
            searchString == label -> midSymbol
            searchString < label -> searchForSymbol(searchString, symbols, low, mid)
            else -> searchForSymbol(searchString, symbols, mid + 1, high)
        }
    }
    ...
    private fun getIdentifierSymbol(idString: String): Symbol {
        return searchForSymbol(idString, reservedWords, 0, reservedWords.size) ?: Symbol.identifier
    }
    ...
    private fun skipComment() {
        ...
        while (source.currentChar.toChar() != '\n' && source.currentChar != source.EOF) source.advance()
    ...
    private fun scanIdentifier(): String {
        ...
        assert(isLetter(source.currentChar.toChar())) { "Check identifier start for letter at position ${source.charPosition}." }
    ...
    private fun scanIntegerLiteral(): String {
        ...
        assert(isDigit(source.currentChar.toChar())) { "Check integer literal start for digit at position ${source.charPosition}." }
    ...
    private fun scanStringLiteral(): String {
        ...
        assert(source.currentChar.toChar() == '\"') { "Check for opening quote (\") at position ${source.charPosition}." }
    ...
    private fun scanEscapedChar(): String {
        ...
        assert(source.currentChar.toChar() == '\\') { "Check for escape character ('\\') at position ${source.charPosition}." }
    ...
    private fun checkGraphicChar(n: Int) {
        if (n == source.EOF) throw error("End of file reached before closing quote for Char or String literal.")
        else if (n > 0xffff) throw error("Character not in Unicode Basic Multilingual Pane (BMP)")
        else {
            val c = n.toChar()
    ...
    private fun checkEOF() {
        if (source.currentChar == source.EOF) throw error("Unexpected end of file")
    }
}
